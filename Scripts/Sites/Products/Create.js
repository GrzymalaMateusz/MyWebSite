    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            if (data != "Error") {
                ...
                newdiv.setAttribute("class", "product-gallery-preview col-12 col-md-3 col-lg-4");
                var newimg = document.createElement("img");
                newdiv.appendChild(newimg);
                $('#stringofphoto').val($('#stringofphoto').val() + data + "|");
                newimg.src = '/Images/Product/Gallery/' + data;
                newimg.setAttribute("class", "img-fluid");
                newimg.style.borderRadius = "15px";
                newimg.style.border = "9px solid white";
                newdiv.setAttribute("id", "product-gallery-preview-" + data.replace(".", ""));
...
function Removes(name) {
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            if (data != "Error") {
                $("#product-gallery-preview-" + name.replace(".", "")).remove();
                var count = $('#photo-gallery-count').attr("data-counts");
                var int = parseInt(count) - 1;
                $('#photo-gallery-count').attr('data-counts', int)
...
$(document).ready(function () {
    ...
    $(window).on('beforeunload', function () {
        var array = $('#stringofphoto').val().split('|');
